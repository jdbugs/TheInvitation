...
class PresenceLoopStateTest(unittest.IsolatedAsyncioTestCase):
    async def test_state_records_signature(self) -> None:
        composer = StubComposer()
        display = StubDisplay()
        echo = EchoChamber()
        config = PresenceConfig(
        ...
        fragment = AmbientFragment(
            ...
            signature="sigil-test",
        )
        await loop._emit(fragment)
        self.assertEqual(loop._state.last_signature, "sigil-test")
        self.assertEqual(loop._state.last_reason, "input")
        self.assertEqual(loop._state.emissions, 1)
